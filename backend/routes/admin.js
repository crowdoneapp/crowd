...
router.put('/update-role/:userId', verifyAdmin, async (req, res) => {
  try {
    ...
    // 🔥 PRIVILEGED ROLES $30 LOGIC WITH TRANSACTION HISTORY 🔥
    // Ye wo roles hain jinko $30 ka wallet balance milega (Leader, Setup, Super Setup)
    const bonusRoles = ['leader', 'setup', 'super_setup'];
    
    const wasPrivileged = bonusRoles.includes(oldRole);
    const isNowPrivileged = bonusRoles.includes(newRole);

    if (!wasPrivileged && isNowPrivileged) {
      // ✅ User ko privileged role mila, $30 add karo
      ...
      await Transaction.create({
        ...
        amount: 30,
        source: "admin_bonus",
        description: `Promoted to ${newRole.toUpperCase()}: $30 Balance Added`,
        status: "success",
        date: new Date()
      });
      ...
      // ✅ User ko demote kiya gaya (e.g., setup se wapas normal user), $30 deduct karo
      ...
      await Transaction.create({
        ...
        description: `Demoted from ${oldRole.toUpperCase()}: $30 Balance Deducted`,
        status: "success",
        date: new Date()
      });
    }
...
// C:\Users\HP\Desktop\crowdone\backend\routes\admin.js
