...
// Country Code ko Full Name + Flag mein badalne ke liye
// const getFullCountryName = (code) => {
//     const countryMap = {
//         "IN": "🇮🇳 India", "MY": "🇲🇾 Malaysia", "ZA": "🇿🇦 South Africa",
//         "NG": "🇳🇬 Nigeria", "PK": "🇵🇰 Pakistan", "BD": "🇧🇩 Bangladesh",
//         "LK": "🇱🇰 Sri Lanka", "VN": "🇻🇳 Vietnam", "GH": "🇬🇭 Ghana", "KE": "🇰🇪 Kenya"
//     };
//     const upperCode = (code || 'IN').toUpperCase();
//     return countryMap[upperCode] || `🌍 ${upperCode}`;
// };
...
const getFullCountryName = (codeOrName) => {
    ...
    const cleanInput = (codeOrName || 'IN').toString().trim().toUpperCase();
    return countryMap[cleanInput] || `🌍 ${codeOrName}`;
};

const sendTelegramAlert = async (userName, userId, amount, countryCode, txHash = "N/A") => {
    const botToken = process.env.TELEGRAM_BOT_TOKEN;
    const chatId = process.env.TELEGRAM_CHAT_ID;
    ...
    // BscScan Link (Official BEP-20)
    const explorerLink = txHash !== "N/A" ? `https://bscscan.com/tx/${txHash}` : "#";

    // 📩 TELEGRAM HTML MESSAGE TEMPLATE
    // Receipt No. yahan se completely hata diya gaya hai
    const message = `
🚨 <b>NEW DEPOSIT VERIFIED!</b> 🚨

👤 <b>Member:</b> ${userName}
🆔 <b>User ID:</b> <code>${userId}</code>
💰 <b>Amount:</b> <b>$${amount} USDT (BEP-20)</b>
🌍 <b>Country:</b> ${displayCountry}

🔗 <b>Blockchain Transaction:</b>
${txHash !== "N/A" ? `<a href="${explorerLink}">🔍 View on BscScan</a>\n<code>${txHash}</code>` : "<i>Processing...</i>"}

✅ <i>Auto-Verified by CrowdOne System</i>
    `;
    ...
    try {
        ...
        await axios.post(url, {
            ...
            text: message,
            parse_mode: 'HTML',
            disable_web_page_preview: true // BscScan ka bada sa link preview band karne ke liye
        });
        console.log(`✅ Telegram Text Alert sent for User: ${userId} | Amount: $${amount}`);
    } catch (error) {
        console.error("❌ Telegram Send Error:", error.message);
    }
};
