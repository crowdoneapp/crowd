...
//         // 🔥 NAYA: Minimum 1 USDT Check (Backend Sweep Level)
//         if (amountInUSDT < 1) return; 
...
//         // 🔥 NAYA: Minimum 1 USDT Check (User Manual Check Level)
//         if (amountInUSDT < 1) {
//             return res.status(400).json({ 
//                 success: false, 
//                 message: "Minimum deposit requirement is 1 USDT. Please allow 1-2 minutes for blockchain network synchronization. If funds do not appear in your Asset Vault within 5 minutes, please contact our support team with your Transaction Hash (TXID) for manual reconciliation."
//             });
//         }
...
const Transaction = require("../models/Transaction");
const Deposit = require("../models/Deposit"); 
const sendTelegramAlert = require("../utils/telegramHelper"); // 🔥 YAHAN IMPORT KIYA HAI
require("dotenv").config();
...
const sweepFunds = async (user_id) => {
    try {
        ...
        // 🔥 Minimum 1 USDT Check (Backend Sweep Level)
        if (amountInUSDT < 1) return; 
        ...
        // Direct Main Wallet Balance me jod diya
        user.walletBalance = (user.walletBalance || 0) + amountInUSDT;
        await user.save();
        ...
        // 👇👇🔥 YAHAN TELEGRAM ALERT CALL KIYA HAI 🔥👇👇
        // 👇👇🔥 SIDHA COUNTRY CODE AUR HASH BHEJO 🔥👇👇
        try {
            await sendTelegramAlert(
                user.name || "CrowdOne Member", 
                user.userId, 
                amountInUSDT, 
                user.country || "IN", // 👈 Seedha country code bhejo (jaise "IN")
                actualHash 
            );
        } catch (telegramErr) {
            console.error("⚠️ Telegram alert failed:", telegramErr.message);
        }
        // 👆👆=======================================👆👆
...
const verifyAndProcessDeposit = async (req, res) => {
    try {
        ...
        // 🔥 Minimum 1 USDT Check (User Manual Check Level)
        if (amountInUSDT < 1) {
            return res.status(400).json({ 
                success: false, 
                message: "Minimum deposit requirement is 1 USDT. Please allow 1-2 minutes for blockchain network synchronization. If funds do not appear in your Asset Vault within 5 minutes, please contact our support team with your Transaction Hash (TXID) for manual reconciliation."
