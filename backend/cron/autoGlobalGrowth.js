...
// // //                             // 🔥 DOUBLE SECURITY CHECK START 🔥
// // //                             // Yahan hum dobara check kar rahe hain ki usne sach mein directs kiye hain ya nahi
// // //                             const pkgAmount = pool.packageAmount || 30;
// // //                             let userDirectsForPackage = 0;
                            
// // //                             if (pkgAmount === 30) {
// // //                                 userDirectsForPackage = user.packageStats?.[pkgAmount]?.directCount || user.directCount || 0;
// // //                             } else {
// // //                                 userDirectsForPackage = user.packageStats?.[pkgAmount]?.directCount || 0;
// // //                             }

// // //                             // Agar us pool ke level ke barabar directs nahi hain, toh paisa DONT SEND!
// // //                             if (userDirectsForPackage < pool.level) {
// // //                                 console.log(`🚫 Payout blocked for User ${user.userId}. Level ${pool.level} needs ${pool.level} directs. Found: ${userDirectsForPackage}`);
// // //                                 pool.status = 'BLOCKED_NO_DIRECTS'; // Isko permanently block kar diya
// // //                                 isUpdated = true;
// // //                                 continue; // Is pool ko yahin chhod do aur agle pool par jao
// // //                             }
// // //                             // 🔥 DOUBLE SECURITY CHECK END 🔥

// // //                             // Agar security check pass ho gaya, tabhi income add hogi
// // //                             user.poolIncome = (user.poolIncome || 0) + pool.dailyAmount;
...
// //     GLOBAL_POOLS.push({
// //         level: i,
// //         globalTeam: i * 100, // Team crowd requirement: 100, 200, 300...
// //         reqDirects: 0,       // 🔥 Direct requirement completely removed
// //         days: ROI_DAYS
// //     });
// // }
...
// //                     for (let lvl of GLOBAL_POOLS) {
// //                         // 🔥 Yahan se Directs ki condition poori tarah hata di gayi hai
// //                         if (userCrowdForPackage >= lvl.globalTeam) { 
...
// //                             // 🔥 MAIN UPDATE: ONLY PAY OUT IF PACKAGE IS $30 🔥
// //                             if (pkgAmount !== 30) {
// //                                 // Ignore distribution for $100, $300, $500, $1000 packages.
// //                                 // Do not update daysPaid or lastPaidDate so it stays "frozen"
// //                                 continue; 
// //                             }

// //                             // Agar $30 ka package hai, tabhi income add hogi
// //                             user.poolIncome = (user.poolIncome || 0) + pool.dailyAmount;
...
//             // 🔥 NAYA UPDATE: setup aur super_setup ko auto-pool check se bahar nikal diya
...
//         try {
//             console.log("🚀 Starting Daily Community Payouts...");
//             // 🔥 NAYA UPDATE: setup aur super_setup ko payout list se bahar nikal diya
...
for (let i = 1; i <= TOTAL_LEVELS; i++) {
    GLOBAL_POOLS.push({
        level: i,
        globalTeam: i * 100, // Team crowd requirement: 100, 200, 300...
        reqDirects: i,       // 🔥 HAR LEVEL KE LIYE DIRECT ZAROORI HAI (Level 1=1, Level 2=2... Level 50=50)
        days: ROI_DAYS
    });
}
...
const startGlobalGrowthCron = () => {
    ...
    cron.schedule('* * * * *', async () => {
        try {
            ...
            // 🔥 NAYA UPDATE: setup aur super_setup ko auto-pool check se STRICTLY bahar nikal diya
            const eligibleUsers = await User.find({ 
                isToppedUp: true, 
                role: { $nin: ['setup', 'super_setup'] } 
            });
            ...
            for (let user of eligibleUsers) {
                ...
                // 🔥 DIRECT COUNT CHECK: Sirf un directs ko ginega jo active (isToppedUp: true) hain
                ...
                for (let pkg of activePkgs) {
                    ...
                    for (let lvl of GLOBAL_POOLS) {
                        // 🔥 YAHAN DONO SHARTEIN (CROWD + DIRECTS) CHECK HONGY
                        if (userCrowdForPackage >= lvl.globalTeam && activeDirectsCount >= lvl.reqDirects) { 
