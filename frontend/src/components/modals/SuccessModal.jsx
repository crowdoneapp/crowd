...
import React from "react";
import { CheckCircle2, Landmark, ArrowRightLeft, Gift, ShieldCheck, ArrowDownLeft, User, CalendarDays, ArrowUpCircle, Wallet, Globe, Star, Crown, Clock, X } from "lucide-react";
...
const SuccessModal = ({
  ...
  let ribbonText = `${numberToWords(amount)} US DOLLARS`;
  ...
  switch (type) {
    case "withdrawal":
      ...
      ribbonText = `${numberToWords(amount)} US DOLLARS`;
    ...
    case "buy":
      ...
      ribbonText = `FOR ${amount} US DOLLARS`;
    ...
    case "spin":
      ...
      ribbonText = `${numberToWords(reward)} US DOLLARS`;
    ...
    <div className="fixed inset-0 bg-black/90 backdrop-blur-sm flex items-center justify-center p-2 animate-in fade-in duration-300 font-sans" style={{ zIndex }}>
      ...
      <div className="relative w-full max-w-2xl bg-[#020b1c] border-[2px] sm:border-[3px] border-[#d4af37] p-3 sm:p-8 text-white max-h-[98vh] overflow-y-auto hide-scroll shadow-[0_0_40px_rgba(212,175,55,0.15)] outline outline-1 outline-offset-2 outline-[#d4af37]">
        ...
        <div className="text-center mb-4">
          ...
          <h1 className="text-4xl sm:text-5xl md:text-6xl text-[#fceabb] mb-1 sm:mb-2 drop-shadow-xl" style={{ fontFamily: "'Brush Script MT', 'Great Vibes', 'Dancing Script', cursive", fontStyle: "italic" }}>
            {userName}
          </h1>
