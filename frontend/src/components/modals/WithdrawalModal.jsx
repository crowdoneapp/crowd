...
// // import MessageModal from "./MessageModal";
// // import { useAuth } from "../../context/AuthContext";
// // import { Wallet, Zap, Lock, X, UserCog, Eye, EyeOff, ShieldCheck, Activity, Info, AlertCircle } from "lucide-react"; 
// // import { Link } from "react-router-dom"; 
...
// //   const totalWorkingBalance = balances.direct + balances.level + balances.reward + balances.roi + balances.matchingRoi;
// //   const workingAmt = Number(withdrawals.working) || 0;
// //   const totalEnteredAmount = workingAmt;
// //   const isWorkingActive = workingAmt > 0;
...
// //             <div className="absolute top-0 left-0 w-32 h-32 bg-indigo-500/20 blur-[50px] pointer-events-none rounded-full"></div>
// //             <div className="relative z-10 text-center">
// //               <h1 className="text-white text-2xl font-black tracking-tight mb-1">Withdraw Assets</h1>
// //               <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest mt-1">Select Balance to Withdraw</p>
// //             </div>
// //           </div>
...
// //             {isAddressMissing && !isPromo ? (
// //               <div className="bg-rose-500/10 border border-rose-500/30 rounded-2xl p-4 flex flex-col items-center justify-center text-center shadow-sm">
// //                 <p className="text-[11px] text-rose-300 font-bold leading-relaxed mb-3">External Wallet not found! Please update BEP-20 address first.</p>
// //                 <Link to="/profile" onClick={onClose} className="bg-rose-500 hover:bg-rose-600 text-white px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 transition-colors shadow-[0_0_10px_rgba(244,63,94,0.3)] w-full justify-center">
// //                   <UserCog size={14} /> Update Your Usdt Bep-20 Address
// //                 </Link>
// //               </div>
// //             ) : (
...
// //             <div className="bg-white/[0.04] border border-white/10 rounded-2xl px-4 py-3 shadow-inner transition-all group focus-within:border-cyan-500/50 hover:border-cyan-500/30">
// //               <div className="flex justify-between items-center mb-2">
// //                 <h3 className="text-slate-300 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5">
// //                     <Zap size={14} className="text-amber-400" /> Combined  Earning
// //                 </h3>
// //                 <span className="text-[11px] font-mono font-bold text-amber-400">Available: ${totalWorkingBalance.toFixed(2)}</span>
// //               </div>
// //               <div className="flex items-center bg-black/40 border border-white/5 rounded-xl px-3 py-2 shadow-inner">
// //                   <span className="text-amber-400 font-black text-sm pr-1">$</span>
// //                   <input 
...
// //                       className="w-full bg-transparent border-none text-white text-base font-black font-mono outline-none placeholder-slate-600 disabled:opacity-50"
// //                       value={withdrawals.working} onChange={e => handleInputChange(e, "working")} disabled={isLeader} 
// //                   />
// //                   {!isLeader && (
// //                       <button onClick={() => setWithdrawals({ working: Math.floor(totalWorkingBalance) })} className="text-[9px] font-black uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-2 py-1 rounded-md border border-cyan-500/20 active:scale-95 transition-all ml-2">Max</button>
// //                   )}
// //               </div>
// //             </div>
...
// //                   className="w-full bg-white/[0.04] border border-white/10 rounded-2xl px-4 py-3.5 pl-11 pr-11 text-white focus:border-cyan-400/60 focus:ring-2 focus:ring-cyan-500/20 focus:outline-none transition-all font-mono font-bold tracking-widest placeholder-slate-500 text-sm"
// //                 />
// //                 <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-cyan-400 transition-colors focus:outline-none">
// //                   {showPassword ? <EyeOff size={16} /> : <Eye size={16} />}
// //                 </button>
// //               </div>
// //             </div>
...
// //               }`}
// //             >
// //               {loading ? <><Activity size={18} className="animate-pulse" /> Processing...</> : <><Zap size={18} strokeWidth={2.5} /> Withdraw Now</>}
// //             </button>
...
// import MessageModal from "./MessageModal";
// import { useAuth } from "../../context/AuthContext";
// import { Wallet, Zap, Lock, X, UserCog, Eye, EyeOff, ShieldCheck, Activity, Info } from "lucide-react"; 
// import { Link } from "react-router-dom"; 
...
//   const [withdrawals, setWithdrawals] = useState({ working: "" });
...
//           roi: Number(res.data.roi) || 0,
//           isUserToppedUp: res.data.isUserToppedUp || false,
//           topUpAmount: res.data.topUpAmount || 0
//         });
//       }
...
//     const value = e.target.value;
//     if (value === "" || /^\d*\.?\d{0,2}$/.test(value)) {
//       setWithdrawals({ working: value });
//     }
//   };

//   const proceedWithdrawal = async () => {
//     if (isLeader) return showMessage("Action Denied", "System roles cannot withdraw funds directly from here.");
//     if (!balances.isUserToppedUp || balances.topUpAmount < 2) return showMessage("Access Denied", "A minimum $1 package upgrade is required to withdraw.");
//     if (!walletAddress.trim() && isAddressMissing && !isPromo) return showMessage("Address Missing", "Please configure your BEP-20 withdrawal address in your Profile.");
    
//     if (workingAmt === 0) return showMessage("Invalid Request", "Enter an amount to withdraw.");
//     if (workingAmt < 2 || workingAmt > 5000) return showMessage("Limit Error", "Withdrawal must be between $2 and $5000.");
//     if (workingAmt > totalWorkingBalance) return showMessage("Insufficient Funds", "Not enough income balance available.");
//     if (!transactionPassword.trim()) return showMessage("Security Alert", "Enter your Tx-PIN.");
...
//     } catch (err) {
//       let errorMsg = err.response?.data?.message || err.message || "Withdrawal failed.";
//       showMessage("Withdrawal Failed", errorMsg);
//     } finally {
//       setLoading(false); 
//     }
//   };
...
//            type="withdrawal" userId={successData.userId} userName={successData.userName} 
...
//             <div className="relative z-10 text-center">
//               <h1 className="text-white text-2xl font-black tracking-tight mb-1">Withdraw Assets</h1>
//               <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest mt-1">Instant Payouts</p>
//             </div>
//           </div>
...
//             <div className="grid grid-cols-2 gap-3">
//                 <div className="bg-cyan-500/10 border border-cyan-500/20 p-3 rounded-2xl shadow-inner">
//                     <p className="text-cyan-400/80 text-[9px] font-bold uppercase tracking-widest mb-1 text-center">Available Income</p>
//                     <h3 className="text-lg text-center font-black text-white font-mono">${totalWorkingBalance.toFixed(2)}</h3>
//                 </div>
...
//               <div className="flex justify-between items-center mb-2">
//                 <h3 className="text-slate-300 text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5">
//                     <Zap size={14} className="text-amber-400" /> Enter Amount
//                 </h3>
//               </div>
//               <div className="flex items-center bg-black/40 border border-white/5 rounded-xl px-3 py-2 shadow-inner">
//                   <span className="text-amber-400 font-black text-sm pr-1">$</span>
...
//                       type="number" placeholder="0.00" 
//                       className="w-full bg-transparent border-none text-white text-base font-black font-mono outline-none placeholder-slate-600 disabled:opacity-50"
//                       value={withdrawals.working} onChange={handleInputChange} disabled={isLeader} 
//                   />
//                   {!isLeader && (
//                       <button onClick={() => setWithdrawals({ working: Math.floor(totalWorkingBalance) })} className="text-[9px] font-black uppercase tracking-wider text-cyan-400 bg-cyan-500/10 px-2 py-1 rounded-md border border-cyan-500/20 active:scale-95 transition-all ml-2">Max</button>
//                   )}
...
//             <div className={`border rounded-2xl p-4 transition-all duration-300 ${workingAmt > 0 ? 'bg-indigo-500/10 border-indigo-500/30' : 'bg-white/5 border-white/10'}`}>
//                 <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-300 border-b border-white/10 pb-2 mb-3 flex items-center gap-1.5">
//                     <Info size={12} /> Withdrawal Summary
//                 </h4>
//                 <div className="space-y-2 text-xs font-medium">
//                     <div className="flex justify-between">
//                         <span className="text-slate-400">Total Requested:</span>
//                         <span className="text-white font-mono">${workingAmt.toFixed(2)}</span>
//                     </div>
//                     <div className="flex justify-between items-center border-t border-white/10 pt-2 mt-2">
//                         <span className="text-emerald-400 font-bold uppercase tracking-wider text-[10px]">Net Payable:</span>
//                         <span className="text-emerald-400 font-black text-sm font-mono">${workingAmt.toFixed(2)}</span>
//                     </div>
//                 </div>
//             </div>
...
const WithdrawalModal = ({ userId, onClose }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const u = profileRes.data?.user || profileRes.data; // 🔥 User Profile data yahan hai
      ...
      if (res.data) {
        setBalances({
          ...
          level: Number(res.data.level) || 0,
          roi: Number(res.data.roi) || 0,
          isUserToppedUp: u?.isToppedUp || false, // 🔥 FIX: Profile se direct fetch ho raha hai
          topUpAmount: u?.topUpAmount || 0        // 🔥 FIX: Profile se direct fetch ho raha hai
        });
      }
  ...
  const proceedWithdrawal = async () => {
    ...
    // 🔥 FIX: Check < 2 aur Message $2 kiya gaya hai
    if (!balances.isUserToppedUp || balances.topUpAmount < 2) return showMessage("Access Denied", "A minimum $2 package upgrade is required to withdraw.");
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[1000] flex justify-center items-center p-4 overflow-hidden animate-in fade-in duration-300">
          ...
          <div className="px-5 pb-6 space-y-4 relative z-10">
            <div className="grid grid-cols-1 gap-3">
                <div className="bg-cyan-500/10 border border-cyan-500/20 p-4 rounded-2xl shadow-inner">
                    <p className="text-cyan-400/80 text-[10px] font-bold uppercase tracking-widest mb-1 text-center">Available Income</p>
                    <h3 className="text-2xl text-center font-black text-white font-mono">${totalWorkingBalance.toFixed(2)}</h3>
                </div>
            </div>
