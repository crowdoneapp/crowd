...
//   // 2. 🔥 NAYA: ROI & Actual Pool Income Fetching
...
//   // 3. Staking Incomes (Agar future me use ho toh as it is rakha hai)
//   const cctStakingIncome = Number(income.cctStakingIncome) || Number(user?.cctStakingIncome) || 0;
//   const cctStakingDirectIncome = Number(income.cctStakingDirectIncome) || Number(user?.cctStakingDirectIncome) || 0;
//   const cctStakingLevelIncome = Number(income.cctStakingLevelIncome) || Number(user?.cctStakingLevelIncome) || 0;

//   // 🔥 4. Crowd Donation Earning (50-Level Package-Wise Calculation)
//   useEffect(() => {
//     if (!user) return;
...
//   // 🔥 MAIN UPDATE: Ab Potential Income + Actual Pool Income + ROI Income tino add honge
//   const combinedCrowdDonation =   actualPoolIncome + roiIncome;

//   // 5. UPDATED TOTAL EARNING (Total Matching with IncomeSummary)
//   const totalEarning =
//     directIncome +
//     levelIncome +
//     combinedCrowdDonation + // Yahan ROI aur Pool dono aa gaye
//     cctStakingIncome +
//     cctStakingDirectIncome +
//     cctStakingLevelIncome;
...
const WalletBalance = ({ income = {} }) => {
  ...
  // 2. ROI & Actual Pool Income Fetching
  const roiIncome = Number(income.totalRoiIncome) || Number(user?.totalRoiIncome) || Number(user?.roiIncome) || 0;
  ...
  // 4. TOTAL EARNING (Sirf actual incomes, extra crowd donation hata diya gaya hai)
  const totalEarning =
    directIncome +
    levelIncome +
    roiIncome +
