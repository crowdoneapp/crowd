...
const UserListTable = () => {
  ...
  const filteredUsers = useMemo(() => {
    return users.filter(user => {
      ...
      // ✅ SABHI FIELDS ME SEARCH LOGIC ADD KAR DIYA
      const nameMatch = user.name?.toLowerCase().includes(searchLower);
      const idMatch = String(user.userId).toLowerCase().includes(searchLower);
      const sponsorMatch = String(user.sponsorId || '').toLowerCase().includes(searchLower);
      const depositMatch = String(user.depositAddress || '').toLowerCase().includes(searchLower);
      const walletMatch = String(user.walletAddress || '').toLowerCase().includes(searchLower);
      const mobileMatch = String(user.mobile || '').toLowerCase().includes(searchLower);
      const emailMatch = String(user.email || '').toLowerCase().includes(searchLower);
      ...
      // TopUp Filter Logic (Updated for all packages)
      let topUpMatch = true;
      const amount = user.topUpAmount || 0;
      ...
      // 🔥 UPDATE: Kisi ek field me bhi match hua toh user dikhega
      return (nameMatch || idMatch || sponsorMatch || depositMatch || walletMatch || mobileMatch || emailMatch) && inDateRange && topUpMatch;
    });
  }, [users, search, dateFrom, dateTo, topUpFilter]); // Sirf inke change hone par chalega
    ...
    <div className="p-4 ">
      ...
      <div className="flex flex-col pt-12 xl:flex-row xl:items-center xl:justify-between gap-4 mb-6">
        ...
        <div className="flex flex-col md:flex-row gap-2 w-full xl:w-auto flex-wrap">
          <input
            type="text"
            className="border text-black border-gray-300 rounded px-3 py-2 w-full md:w-72"
            placeholder="Search Name/ID/Mobile/Email/Address..." 
            value={search}
            onChange={e => setSearch(e.target.value)}
          />
          <input
          ...
          {/* 🔥 Package Filter Dropdown (Updated with accurate packages) */}
          <select 
            className="border border-gray-300 rounded px-3 py-2 bg-white font-medium text-gray-700"
            value={topUpFilter}
            onChange={(e) => setTopUpFilter(e.target.value)}
          >
            ...
            <option value="300">$300 Package</option>
            <option value="500">$500 Package</option>
            <option value="1000">$1000 Package</option>
          </select>
            ...
            {currentItems.length === 0 ? (
              ...
              currentItems.map((user, idx) => (
                <tr key={idx} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-2 border">
                    <div className="flex items-center gap-2">
                      <span className="text-blue-600 font-bold">{user.userId}</span>
                      <button 
                        onClick={() => handleCopy(user.userId.toString())}
                        title="Copy User ID"
                        className="text-black hover:text-gray-700 transition"
