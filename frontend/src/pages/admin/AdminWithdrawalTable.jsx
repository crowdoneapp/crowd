...
const AdminWithdrawalTable = () => {
    ...
    let extractedData = nonLeaderWithdrawals.flatMap(w => {
     ...
     const createdAt = w.date ? new Date(w.date) : new Date(w.createdAt);
      ...
      return {
        ...
        status: w.status ?? 'pending',
        createdAt,
        isFirstWithdrawal: w.isFirstWithdrawal 
      };
    });
  ...
  // 🔥 NAYA UPDATE: Yahan Approved ki calculation add ki gayi hai
  const totals = useMemo(() => {
    return filteredData.reduce(
      (acc, w) => {
        if (w.status === 'pending') {
          ...
          acc.approvedCount += 1;
          acc.approvedNet += w.netAmount || 0;
        }
        return acc;
      },
      { pendingCount: 0, pendingGross: 0, pendingFee: 0, pendingNet: 0, approvedCount: 0, approvedNet: 0 }
    );
  }, [filteredData]);
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow">
      ...
      <div className="flex flex-wrap items-center gap-4 mb-6 bg-slate-50 p-4 rounded-lg border border-slate-200">
        ...
        {/* 🔥 NAYA UPDATE: Quick Date Filter Buttons */}
        <div className="flex items-center gap-2 border-r border-slate-300 pr-4">
          <button onClick={() => setQuickDate(0, 0)} className="bg-slate-200 text-slate-700 text-xs font-bold px-3 py-2 rounded hover:bg-slate-300 transition">Today</button>
          <button onClick={() => setQuickDate(1, 1)} className="bg-slate-200 text-slate-700 text-xs font-bold px-3 py-2 rounded hover:bg-slate-300 transition">Yesterday</button>
          <button onClick={() => setQuickDate(3, 0)} className="bg-slate-200 text-slate-700 text-xs font-bold px-3 py-2 rounded hover:bg-slate-300 transition">Last 3 Days</button>
        </div>
      ...
      {/* 🔥 NAYA UPDATE: Total Pending & Total Approved Summary */}
      {!loading && paginatedData.length > 0 && (
         <div className="mt-6 p-4 bg-slate-50 border-2 border-dashed border-slate-300 rounded-lg flex flex-col md:flex-row justify-end items-center gap-6 text-sm font-black uppercase tracking-widest">
            <div className="flex items-center gap-2">
               <span className="text-slate-500">Total Pending Count:</span> 
               <span className="text-blue-600 text-lg bg-blue-50 px-3 py-1 rounded border border-blue-200">{totals.pendingCount}</span>
            </div>
            <div className="flex items-center gap-2">
               <span className="text-slate-500">Total Net Pending:</span> 
               <span className="text-amber-600 text-xl bg-amber-50 px-3 py-1 rounded border border-amber-200">${totals.pendingNet.toFixed(2)}</span>
            </div>
            ...
            <div className="flex items-center gap-2">
               <span className="text-slate-500">Total Net Approved:</span> 
               <span className="text-emerald-600 text-xl bg-emerald-50 px-3 py-1 rounded border border-emerald-200">${totals.approvedNet.toFixed(2)}</span>
            </div>
         </div>
      )}
