...
import {
  ArrowLeft, Lock, User, Mail, Wallet, ShieldCheck, BadgeInfo,
  Settings2, Clock, Fingerprint, ChevronRight
} from 'lucide-react';
import { useAuth } from '../../context/AuthContext';
import MessageModal from '../../components/modals/MessageModal';
...
function UserProfile() {
  ...
  // Wallet update states (password based, no OTP)
  ...
  const handleWalletUpdate = async () => {
    if (!newWalletAddress || !walletPassword) {
      return showMessage('Missing Fields', 'Please enter wallet address and your password.', 'warning');
    }
    setWalletUpdating(true);
    try {
      const res = await api.put(`/user/update-profile-secure`, {
        newWalletAddress: newWalletAddress.trim(),
        password: walletPassword,
      });
      showMessage('Wallet Updated', 'Your wallet address has been updated successfully.', 'success');
      setFormData((prev) => ({ ...prev, walletAddress: res.data.user.walletAddress }));
      setNewWalletAddress('');
      setWalletPassword('');
    } catch (err) {
      showMessage('Error', err.response?.data?.message || 'Wallet update failed.', 'error');
    } finally {
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6 animate-in fade-in slide-in-from-right-2 duration-300">
                ...
                <div className="bg-white border border-slate-100 shadow-[0_8px_30px_-10px_rgba(0,0,0,0.04)] rounded-[24px] p-6 md:p-8">
                  ...
                  {/* Set / Update Wallet Address (password based, in-app) */}
                  {!formData.walletAddress && (
                    <div className="mt-6 bg-blue-50/80 border border-blue-100 rounded-2xl p-5 md:p-6 shadow-sm">
                      <h4 className="text-blue-800 font-black text-sm mb-2 flex items-center gap-2">
                        <BadgeInfo size={16} /> Set Your Wallet Address
                      </h4>
                      <p className="text-xs font-semibold text-blue-700/80 mb-5 leading-relaxed">
                        Enter your USDT (BEP20) wallet address and confirm with your login password.
                      </p>
                      ...
                      <div className="space-y-3">
                        <input
                          ...
                          className="w-full bg-white border border-blue-200 rounded-xl px-4 py-3.5 text-slate-900 focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none text-sm font-mono font-bold placeholder-slate-400 transition-all shadow-sm"
                        />
                        <input
                          type="password"
                          ...
                          className="w-full bg-white border border-blue-200 rounded-xl px-4 py-3.5 text-slate-900 focus:border-blue-400 focus:ring-4 focus:ring-blue-50 outline-none text-sm font-bold placeholder-slate-400 transition-all shadow-sm"
                        />
                        <button
                          onClick={handleWalletUpdate}
                          disabled={walletUpdating}
                          className="inline-flex items-center justify-center gap-2 w-full bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-black text-[11px] uppercase tracking-widest py-3.5 px-6 rounded-xl transition-all active:scale-95 shadow-md disabled:opacity-60"
                        >
                          <Wallet size={16} /> {walletUpdating ? 'Saving...' : 'Save Wallet Address'}
                        </button>
                      </div>
                    </div>
                  )}
