...
const LevelIncome = () => {
  ...
  const handlePrev = () => currentPage > 1 && setCurrentPage((p) => p - 1);
  const handleNext = () => currentPage < totalPages && setCurrentPage((p) => p - 1 + 2); // logical fix for increment
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 px-2">
        <div>
          ...
          <p className="text-slate-500 text-xs md:text-sm font-semibold tracking-widest uppercase mt-2 ml-1">
            Track your team generation earnings
          </p>
        </div>
      </div>
            ...
            <div className="space-y-2.5">
              {loading ? (
                ...
                paginated.map((txn, idx) => {
                  ...
                  // Yahan guaranteed ek valid doosri ID hogi
                  const displayUser = getValidSourceUser(txn);
