...
// ✅ Light Theme Styles (Matching your App)
const customStyles = `
  .neo-bg {
...
function UserWithdrawalHistory() {
  ...
  const groupWithdrawals = (list) => {
    ...
    for (const item of sorted) {
      ...
      const iGross = isNewLogic ? Number(item.grossAmount || 0) : Number(item.grossAmount || 0) * 2;
      const iFee = isNewLogic ? Number(item.fee || 0) : Number(item.fee || 0) * 2;
      const iCryptoNet = Number(item.netAmount || 0); 
      const iTopupNet = isNewLogic ? (iGross - iFee - iCryptoNet) : Number(item.netAmount || 0);
  ...
  // 🔥 UPDATE: PENDING AUR APPROVED DONO KA TOTAL
  const totalAmount = grouped.reduce((sum, g) => {
  ...
  const startIdx = (currentPage - 1) * itemsPerPage;
  const paginated = sortedFiltered.slice(startIdx, startIdx + itemsPerPage);
  ...
  const getStatusDetails = (group) => {
    const s = group.status?.toLowerCase();
    if (s === "approved" || s === "success") {
      return { label: group.status, color: "bg-emerald-100 text-emerald-700 border border-emerald-200" };
    }
    if (s === "pending") {
      return { label: group.status, color: "bg-amber-100 text-amber-700 border border-amber-200" };
    }
    if (s === "rejected" || s === "failed") {
      return { label: group.status, color: "bg-rose-100 text-rose-700 border border-rose-200" };
    }
    return { label: group.status || "UNKNOWN", color: "bg-slate-100 text-slate-600 border border-slate-200" };
  };
  ...
  return (
    <div className="neo-bg min-h-screen w-full py-6 md:py-10 text-slate-800 relative font-sans">
      ...
      <div className="relative z-10 w-full px-4 sm:px-6 md:px-8 max-w-7xl mx-auto pb-24">
        ...
        {/* STATS CARD (MATCHING THE THEME SCREENSHOT) */}
        <div className="mb-8">
          <div className="relative overflow-hidden bg-gradient-to-br from-[#1a56db] to-[#2563eb] p-6 md:p-8 rounded-[24px] shadow-lg shadow-blue-600/20 max-w-md group transition-transform hover:-translate-y-1 duration-300">
            
            <div className="absolute -right-10 -top-10 w-32 h-32 bg-blue-300/30 rounded-full blur-[35px] pointer-events-none"></div>

            <h3 className="text-blue-100 text-[10px] md:text-xs font-bold uppercase tracking-widest flex items-center gap-2 mb-2">
              <Wallet size={14} className="text-blue-200" />
              Total Withdrawn (Pending + Approved)
            </h3>

            <p className="text-3xl md:text-4xl font-black text-white font-mono drop-shadow-sm">
              ${totalAmount.toFixed(2)}
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-center bg-white shadow-sm p-4 rounded-2xl border border-slate-200">
          ...
          <div className="relative w-full sm:w-80 group">
            ...
            <input
              ...
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 text-slate-800 text-sm font-bold tracking-wide rounded-xl px-4 py-3 pl-11 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all placeholder-slate-400"
            />
          </div>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <select
              value={statusFilter}
              onChange={(e) => setStatusFilter(e.target.value)}
              className="w-full sm:w-auto bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all appearance-none cursor-pointer capitalize outline-none"
            >
        ...
        <div className="w-full neo-table-wrapper">
          <div className="overflow-x-auto crowd-scroll w-full">
            <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
              ...
              <thead className="bg-[#0b1c3c] text-white text-[11px] md:text-xs font-bold uppercase tracking-wider">
                <tr>
                  <th className="py-4 px-5 text-center">Sr.</th>
                  <th className="py-4 px-5">Date</th>
                  <th className="py-4 px-5">Total Amount</th>
                  <th className="py-4 px-5">Fee (10%)</th>
                  <th className="py-4 px-5">
                    <span className="text-blue-300">BEP-20 Output</span>
                  </th>
                  <th className="py-4 px-5">Linked Address</th>
                ...
                {loading ? (
                  ...
                  paginated.map((group, idx) => {
                    ...
                    return (
                      ...
                      >
                        <td className="py-4 px-5 font-bold text-slate-500 text-center">
                          {startIdx + idx + 1}
                        </td>

                        <td className="py-4 px-5 text-slate-600 font-mono text-[12px] sm:text-sm font-semibold tracking-wide">
                          {group.createdAt ? format(new Date(group.createdAt), "dd MMM yyyy") : "N/A"}
                        </td>

                        <td className="py-4 px-5 font-black text-[#0b1c3c] font-mono text-sm">
                          ${group.totalGross.toFixed(2)}
                        </td>

                        <td className="py-4 px-5 font-bold text-rose-500 font-mono">
                          -${group.totalFee.toFixed(2)}
                        </td>

                        <td className="py-4 px-5 font-black text-emerald-600 font-mono">
                          ${group.totalCryptoNet.toFixed(2)}
                        </td>

                        <td className="py-4 px-5 text-slate-500 font-mono text-[10px] sm:text-[11px]">
                          {group.walletAddress ? (
                            <span className="px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50">
                              {group.walletAddress}
                            </span>
                          ) : (
          ...
          {/* PAGINATION */}
          {!loading && sortedFiltered.length > 0 && (
            <div className="bg-[#f8fafc] border-t border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">

              <span className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">
                Showing <span className="text-[#0b1c3c] font-black">{startIdx + 1}</span> to{" "}
                <span className="text-[#0b1c3c] font-black">
                  {Math.min(startIdx + itemsPerPage, sortedFiltered.length)}
                </span>{" "}
