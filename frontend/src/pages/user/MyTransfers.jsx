          ...
          <div className="overflow-x-auto crowd-scroll w-full">
            <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
              <thead className="bg-[#0b1c3c] text-white text-[11px] md:text-xs font-bold uppercase tracking-wider">
                <tr>
                  <th className="p-5 text-center w-16">Sr.</th>
                  <th className="p-5">Date</th>
                  <th className="p-5">Transaction Type</th>
                  <th className="p-5">Sender UserId</th>
                  <th className="p-5">Receiver UserId</th>
              ...
              <tbody className="text-slate-700 font-medium">
                {loading ? (
                  <tr>
                    <td colSpan="6" className="text-center py-16 bg-white">
                       <div className="flex flex-col items-center justify-center gap-3">
                          ...
                          <span className="text-[10px] font-bold uppercase tracking-widest text-slate-400">Syncing P2P Ledger...</span>
                       </div>
                    </td>
                  </tr>
                ) : paginatedTransfers.length === 0 ? (
                  ...
                  paginatedTransfers.map((txn, idx) => {
                    ...
                    return (
                      <tr key={txn._id || idx} className="bg-white hover:bg-slate-50 border-b border-slate-100 transition-colors">
                        <td className="p-5 font-bold text-slate-500 text-center">{(currentPage - 1) * itemsPerPage + idx + 1}</td>

                        {/* 🔥 SIRF DATE DIKHEGI, TIME REMOVED */}
                        <td className="p-5 text-slate-600 font-mono text-[12px] sm:text-sm font-semibold tracking-wide">
                          {txn.createdAt ? date.toLocaleDateString("en-GB", { day: '2-digit', month: 'short', year: 'numeric' }) : 'N/A'}
                        </td>
                        ...
                        <td className="p-5 font-mono text-slate-700 text-xs">
                        ...
                        <td className="p-5 font-black text-center text-sm font-mono">
                          <span className={isSent ? "text-rose-500" : "text-emerald-600"}>
                            {isSent ? "-" : "+"} ${Number(txn.amount || 0).toFixed(2)}
                          </span>
                        </td>
                      </tr>
          ...
          {!loading && searchedTransfers.length > 0 && (
             <div className="bg-[#f8fafc] border-t border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-b-16">
                <span className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">
                  Showing <span className="text-[#0b1c3c] font-black">{(currentPage - 1) * itemsPerPage + 1}</span> to <span className="text-[#0b1c3c] font-black">{Math.min(currentPage * itemsPerPage, searchedTransfers.length)}</span> of <span className="text-[#0b1c3c] font-black">{searchedTransfers.length}</span>
                </span>
                ...
                <div className="flex items-center gap-2">
                   <button
                     onClick={handlePrev}
                     disabled={currentPage === 1}
                     className={`p-2.5 rounded-xl flex items-center justify-center transition-all ${currentPage === 1 ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 shadow-sm'}`}
                   >
                     <ChevronLeft size={16} strokeWidth={2.5} />
                   </button>
                   ...
                   <button
                     onClick={handleNext}
                     disabled={currentPage === totalPages}
                     className={`p-2.5 rounded-xl flex items-center justify-center transition-all ${currentPage === totalPages ? 'bg-slate-100 text-slate-300 cursor-not-allowed' : 'bg-white text-slate-600 hover:bg-slate-50 border border-slate-200 shadow-sm'}`}
                   >
                     <ChevronRight size={16} strokeWidth={2.5} />
                   </button>
                </div>
