...
import api from "../../api/axios";
import { format } from "date-fns";
import { Search, ChevronLeft, ChevronRight, Layers, Target, Activity, CheckCircle2, CalendarDays, TrendingUp, Clock } from "lucide-react";
...
const CrowdDonationEarning = () => {
  ...
  return (
    ...
    <div className="w-full max-w-7xl mx-auto p-4 md:p-8 relative z-10 animate-in fade-in duration-500 rounded-3xl bg-white shadow-[0_10px_40px_-10px_rgba(0,0,0,0.08)] border border-slate-100 overflow-hidden font-sans mt-10 md:mt-2">
      ...
      {/* Header */}
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 gap-4 border-b border-slate-100 pb-6">
        <div>
        ...
        <div className="bg-slate-50 border border-slate-200 px-6 py-4 rounded-2xl flex flex-col items-end shadow-sm relative overflow-hidden">
           <div className="absolute top-0 right-0 w-20 h-20 bg-blue-100/50 blur-[30px] rounded-full"></div>
           <span className="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1 relative z-10">Total Donated to You</span>
           <span className="text-blue-600 font-black text-2xl relative z-10 font-mono tracking-tight">${totalEarned.toFixed(2)}</span>
        </div>
      </div>
      ...
      {summary.length > 0 && (
        <div className="mb-10">
            <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest mb-5 flex items-center gap-2">
               <TrendingUp size={16} className="text-blue-500" /> Active Level Trackers
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                {summary.map((pool, idx) => {
                    ...
                    return (
                        <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-5 shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] relative overflow-hidden group hover:border-blue-300 transition-colors">
                            {/* Decorative Background */}
                            <div className="absolute -top-10 -right-10 w-24 h-24 bg-blue-50 blur-[40px] rounded-full group-hover:bg-blue-100 transition-colors"></div>
                            
                            <div className="flex justify-between items-start mb-4 relative z-10">
                                <div>
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="bg-slate-100 text-slate-600 text-[9px] font-black px-2 py-0.5 rounded uppercase tracking-widest border border-slate-200">
                                            Level {pool.level}
                                        </span>
                                        {isCompleted ? (
                                            <span className="bg-emerald-50 text-emerald-600 border border-emerald-200 text-[9px] font-black px-2 py-0.5 rounded flex items-center gap-1 uppercase tracking-widest">
                                                <CheckCircle2 size={10} /> Completed
                                            </span>
                                        ) : (
                                            <span className="bg-blue-50 text-blue-600 border border-blue-200 text-[9px] font-black px-2 py-0.5 rounded flex items-center gap-1 uppercase tracking-widest animate-pulse">
                                                <Activity size={10} /> Active
                                            </span>
                                        )}
                                    </div>
                                    <h4 className="text-lg font-black text-slate-800 font-mono tracking-tight">${pool.tier} <span className="text-slate-400 text-xs font-sans tracking-wide">Package</span></h4>
                                </div>
                                <div className="text-right">
                                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-0.5">Daily</p>
                                    <p className="text-emerald-500 font-black font-mono">+${pool.dailyAmount.toFixed(2)}</p>
                                </div>
                            </div>
                            ...
                            <div className="relative z-10 mb-4">
                                ...
                                <div className="w-full bg-slate-100 rounded-full h-2.5 border border-slate-200 p-0.5 shadow-inner">
                                    <div 
                                        className={`h-full rounded-full transition-all duration-1000 ${isCompleted ? 'bg-emerald-400 shadow-sm' : 'bg-blue-500 shadow-sm'}`}
                                        style={{ width: `${percent}%` }}
                                    ></div>
                                </div>
                                <p className="text-right text-[9px] font-bold text-slate-400 mt-1.5 font-mono">{percent}% Paid</p>
                            </div>
                            ...
                            <div className="pt-4 border-t border-slate-100 flex justify-between items-center relative z-10">
                                ...
                                <div className="text-slate-600 font-mono text-[11px] font-bold tracking-wide">
                                    {pool.lastDate ? format(new Date(pool.lastDate), "dd MMM yyyy") : 'N/A'}
                                </div>
                            </div>
                        </div>
      ...
      <div>
        <h3 className="text-sm font-black text-slate-700 uppercase tracking-widest mb-5 flex items-center gap-2">
           <Layers size={16} className="text-blue-500" /> Daily Credit Logs
        </h3>

        {/* Filters */}
        <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-center bg-slate-50 shadow-sm p-3 rounded-2xl border border-slate-200">
            <div className="relative w-full sm:w-80 group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                ...
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full bg-white border border-slate-300 text-slate-700 text-xs font-bold tracking-wide rounded-xl px-4 py-3 pl-11 focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all placeholder-slate-400 shadow-inner"
                />
            </div>
            ...
            <select
                value={itemsPerPage}
                onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }}
                className="w-full sm:w-auto bg-white border border-slate-300 text-slate-700 text-xs font-bold rounded-xl px-4 py-3 focus:border-blue-400 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all appearance-none cursor-pointer shadow-inner outline-none"
            >
        ...
        <div className="w-full bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="min-w-[800px]">
                <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
                <thead className="bg-slate-50 border-b border-slate-200 text-slate-500 text-[11px] md:text-xs font-black uppercase tracking-widest">
                    <tr>
                    ...
                    <th className="p-4 text-center">Level</th>
                    <th className="p-4 text-center">Day</th>
                    <th className="p-4 text-right">Amount Credit</th>
                    </tr>
                </thead>
                <tbody className="text-slate-700">
                    {loading ? (
                    <tr>
                        <td colSpan="6" className="text-center py-16 bg-white border-b border-slate-100">
                            <div className="flex flex-col items-center justify-center gap-3">
                                <Activity size={28} className="text-blue-500 animate-pulse" />
                                <span className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">Syncing Crowd Data...</span>
                            </div>
                        </td>
                    </tr>
                    ) : paginated.length === 0 ? (
                    ...
                    paginated.map((txn, idx) => {
                        return (
                        <tr key={txn._id || idx} className="bg-white hover:bg-slate-50/50 border-b border-slate-100 transition-colors">
                            <td className="p-4 font-bold text-slate-400 text-center">{indexOfFirst + idx + 1}</td>

                            <td className="p-4 text-slate-600 font-mono text-[11px] sm:text-[12px] font-bold tracking-wide">
                              <div className="flex items-center gap-2">
                            ...
                            <td className="p-4 text-center font-mono text-slate-500 text-xs bg-slate-50 rounded-lg m-2 inline-block px-3 py-1 border border-slate-100">
                                {txn.parsedDay !== 0 ? `${txn.parsedDay} / ${txn.parsedTotalDays}` : "-"}
                            </td>

                            <td className={`p-4 font-black text-right text-sm font-mono text-emerald-500`}>
                              +${txn.parsedAmount.toFixed(2)}
                            </td>
                        </tr>
                        );
            ...
            {!loading && filtered.length > 0 && (
                <div className="p-4 bg-slate-50 border-t border-slate-200 flex flex-col sm:flex-row justify-between items-center gap-4">
                    <span className="text-slate-500 text-[10px] md:text-xs font-black uppercase tracking-widest">
                        Showing <span className="text-blue-600">{indexOfFirst + 1}</span> to <span className="text-blue-600">{Math.min(indexOfLast, filtered.length)}</span> of <span className="text-blue-600">{filtered.length}</span> Logs
                    </span>
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        onClick={handlePrev}
                        disabled={currentPage === 1}
                        className={`p-2 rounded-lg flex items-center justify-center transition-all ${currentPage === 1 ? 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200' : 'bg-white text-slate-600 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 border border-slate-300 shadow-sm'}`}
                        >
                        <ChevronLeft size={16} strokeWidth={2.5} />
                        </button>
                        
                        <span className="bg-white border border-slate-300 text-slate-700 text-xs font-bold px-4 py-2 rounded-lg shadow-sm">
                        ...
                        onClick={handleNext}
                        disabled={currentPage === totalPages}
                        className={`p-2 rounded-lg flex items-center justify-center transition-all ${currentPage === totalPages ? 'bg-slate-100 text-slate-400 cursor-not-allowed border border-slate-200' : 'bg-white text-slate-600 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 border border-slate-300 shadow-sm'}`}
                        >
                        <ChevronRight size={16} strokeWidth={2.5} />
                        </button>
                    </div>
