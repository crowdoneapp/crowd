      ...
      <div className="relative z-10 w-full px-4 sm:px-6 md:px-8 max-w-7xl mx-auto pb-24">
        ...
        <div className="mb-6 relative group w-full sm:w-[400px]">
           ...
           <input
             ...
             value={search}
             onChange={(e) => setSearch(e.target.value)}
             className="w-full bg-white border border-slate-200 text-slate-800 text-sm font-semibold rounded-xl px-5 py-3.5 pl-12 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all placeholder-slate-400 shadow-sm"
           />
        </div>
            ...
            <div className="min-w-[800px]">
              <table className="w-full text-xs sm:text-sm text-left whitespace-nowrap">
                <thead className="bg-[#0b1c3c] text-white text-[11px] md:text-xs font-bold uppercase tracking-wider">
                  <tr>
                    <th className="p-5 text-center">Sr.</th>
                    <th className="p-5">Date</th>
                    <th className="p-5">Type</th>
                    <th className="p-5 text-center">TxHash (BscScan)</th>
                ...
                <tbody className="text-slate-700 font-medium">
                  {loading ? (
                    <tr>
                      <td colSpan="5" className="text-center py-16 bg-white">
                        <div className="flex flex-col items-center gap-3">
                           ...
                           <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Syncing Records...</span>
                        </div>
                      </td>
                    </tr>
                  ) : filteredDeposits.length === 0 ? (
                    ...
                    filteredDeposits.map((record, index) => {
                      ...
                      return (
                        <tr key={record._id || index} className="bg-white hover:bg-slate-50 border-b border-slate-100 transition-colors group">
                          <td className="p-5 font-bold text-slate-500 text-center group-hover:text-blue-600 transition-colors">
                            {index + 1}
                          </td>
                          ...
                          <td className="p-5">
                            <span className="inline-flex items-center gap-1.5 bg-emerald-50 text-emerald-600 border border-emerald-200 py-1.5 px-3 rounded-lg text-[10px] font-black tracking-widest uppercase">
                              <ArrowDownCircle size={14} /> Deposit
                            </span>
                          </td>
                          ...
                          <td className="p-5 text-center">
                            <a 
                              ...
                              target="_blank" 
                              rel="noopener noreferrer"
                              className="inline-flex items-center justify-center gap-1.5 font-mono text-[11px] font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 border border-blue-200 px-3 py-1.5 rounded-lg transition-all"
                            >
                              {shortHash} <ExternalLink size={12} />
                            </a>
                          </td>

                          <td className="p-5 font-black text-center text-emerald-600 text-base">
                            + ${finalAmount.toFixed(2)}
                          </td>
                        </tr>
                      );
