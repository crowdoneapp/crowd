...
import api from "../../api/axios";
import { format } from "date-fns";
import { Search, ChevronLeft, ChevronRight, History, ArrowDownLeft, ArrowUpRight, ArrowRightLeft, Zap, Landmark, Activity } from "lucide-react";

// ✅ Light Neo-Banking Theme (Matching the App)
const customStyles = `
  .neo-bg {
      ...
      switch (txn.type) {
        ...
        case "transfer":
          if (toId === myId) {
            ...
            operator = "-";
            displayTypeUI = "SENT P2P";
            icon = <ArrowRightLeft size={14} className="text-rose-600" />;
          }
          break;
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-center bg-white shadow-sm p-4 rounded-2xl border border-slate-200">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            <select
              value={typeFilter}
              onChange={(e) => setTypeFilter(e.target.value)}
              className="w-full sm:w-auto bg-slate-50 border border-slate-200 text-slate-700 text-sm font-bold rounded-xl px-4 py-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none transition-all appearance-none cursor-pointer uppercase outline-none"
            >
              {dropdownTypes.map((type) => (
        ...
        <div className="w-full neo-table-wrapper">
            ...
            <div className="min-w-[950px]">
              ...
              <div>
                {loading ? (
                  ...
                  <div className="bg-rose-50 py-10 text-center border-b border-rose-100">
                    <span className="text-rose-500 font-bold text-sm uppercase tracking-widest">{error}</span>
                  </div>
                ) : currentItems.length === 0 ? (
                  <div className="bg-white py-16 text-center">
                  ...
                  currentItems.map((txn, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-slate-500 font-mono text-[11px] sm:text-xs font-semibold tracking-wide text-right">
                          {txn.date ? format(new Date(txn.date), "dd MMM yyyy, hh:mm a") : "N/A"}
                        </div>

                        <div>
                          <span className="inline-flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-2.5 py-1.5 rounded-lg text-[10px] font-black tracking-widest text-slate-600">
                            {txn.icon} {txn.displayTypeUI}
                          </span>
                        </div>
          ...
          {!loading && !error && filtered.length > 0 && (
            <div className="bg-[#f8fafc] border-t border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-center gap-4 rounded-b-16">
              <div className="flex items-center gap-3">
                <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest whitespace-nowrap">Rows:</span>
                <select
                  value={itemsPerPage}
                  onChange={(e) => { setItemsPerPage(Number(e.target.value)); setCurrentPage(1); }}
                  className="bg-white border border-slate-200 text-slate-700 text-xs font-bold rounded-lg px-3 py-2 focus:border-blue-500 outline-none cursor-pointer shadow-sm"
                >
                  <option value={10}>10</option>
                  <option value={20}>20</option>
                  <option value={50}>50</option>
              ...
              <span className="text-slate-500 text-[10px] md:text-xs font-bold uppercase tracking-widest">
                Showing <span className="text-[#0b1c3c] font-black">{indexOfFirstItem + 1}</span> to <span className="text-[#0b1c3c] font-black">{Math.min(indexOfLastItem, filtered.length)}</span> of <span className="text-[#0b1c3c] font-black">{filtered.length}</span>
              </span>
