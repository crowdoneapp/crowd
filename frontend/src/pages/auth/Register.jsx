...
import { 
  ...
  CheckCircle2, XCircle, ArrowRight, Copy, Sparkles, Globe, Wallet, Hexagon 
} from 'lucide-react';
...
function Register() {
 ...
 const countryOptions = [
  ...
  // Neo-Banking Light Theme Custom Select Styles
  ...
  return (
    // Split Screen Layout
    <div className="min-h-screen bg-white flex font-sans selection:bg-blue-500/20 selection:text-blue-900">
      ...
      <div className="hidden lg:flex w-1/2 relative overflow-hidden bg-slate-50 flex-col justify-between p-12 border-r border-slate-200">
         ...
         <div className="absolute top-[-10%] left-[-10%] w-[40vw] h-[40vw] bg-blue-500/10 blur-[100px] rounded-full pointer-events-none"></div>
         <div className="absolute bottom-[-10%] right-[-10%] w-[30vw] h-[30vw] bg-indigo-500/10 blur-[100px] rounded-full pointer-events-none"></div>

         <div className="relative z-10">
            <Link to="/" className="flex items-center gap-3 no-underline group w-fit hover:opacity-80 transition-opacity">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-2 rounded-xl shadow-[0_4px_15px_rgba(37,99,235,0.3)]">
         ...
         <div className="relative z-10 my-auto max-w-lg">
            <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-50 border border-blue-200 text-blue-700 text-xs font-black tracking-wide mb-6 shadow-sm">
              <Sparkles size={14} className="text-blue-500" /> INTELLIGENT NETWORKING
            </div>
            <h1 className="text-5xl font-black text-slate-900 leading-tight mb-6 tracking-tight">
               Establish Your <br/>
               <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-600">Global Portfolio</span>
            </h1>
            <p className="text-slate-500 text-lg leading-relaxed font-medium">
               Join the most advanced tier-based community system. Secure, transparent, and built for structured exponential growth.
            </p>
         </div>

         <div className="relative z-10 flex items-center gap-4 text-slate-400 text-sm font-bold uppercase tracking-wider">
      ...
      <div className="w-full lg:w-1/2 flex flex-col justify-center items-center p-4 sm:p-8 relative overflow-y-auto bg-white">
         ...
         <div className="lg:hidden w-full max-w-md mb-6 flex flex-col sm:flex-row justify-between items-center gap-4 mt-2">
            <Link to="/" className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2 no-underline hover:opacity-80 transition-opacity">
              <div className="bg-gradient-to-br from-blue-600 to-indigo-700 p-1.5 rounded-lg shadow-sm">
                <Hexagon size={18} color="white" fill="white" />
              </div>
              CROWD<span className="text-blue-600">ONE</span>
            </Link>
            <Link to="/login" className="text-sm font-black text-blue-700 bg-blue-50 border border-blue-100 hover:bg-blue-100 px-4 py-2 rounded-lg transition-colors shadow-sm">
              Login Instead
            </Link>
         </div>
         ...
         <div className="w-full max-w-md animate-in fade-in slide-in-from-bottom-8 duration-700 mt-2 md:mt-0">
            ...
            <div className="hidden lg:flex justify-end items-center mb-8">
               <Link to="/login" className="text-sm font-bold text-blue-700 bg-blue-50 border border-blue-100 hover:bg-blue-100 px-4 py-2.5 rounded-xl transition-colors shadow-sm">
                 Login Instead
               </Link>
            </div>

            <div className="mb-6 text-center lg:text-left">
              <h2 className="text-3xl sm:text-4xl font-black text-slate-900 mb-2 tracking-tight">Sign Up</h2>
              <p className="text-slate-500 text-sm font-medium">Create your Crowd One account today.</p>
            </div>
            ...
            {errorMsg && (
              <div className="mb-6 bg-rose-50 border border-rose-200 text-rose-600 text-xs p-3.5 rounded-xl font-bold flex items-center gap-2 animate-in slide-in-from-top-2 shadow-sm">
                  <XCircle size={16} className="text-rose-500 min-w-[16px]" /> 
            ...
            <form onSubmit={handleRegister} className="space-y-4">
              ...
              <div className="bg-slate-50/50 p-4 rounded-2xl border border-slate-200 shadow-sm mb-2">
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest ml-1 mb-2 flex items-center gap-2">
                   <Users size={14} className="text-blue-500" /> Referral Sponsor
                </label>
                <div className="relative">
                  <input
                    ...
                    className={`w-full bg-white border ${sponsorName === 'Invalid Sponsor' ? 'border-rose-400 focus:border-rose-500 ring-2 ring-rose-50' : (sponsorName && sponsorId) ? 'border-blue-400 focus:border-blue-500 ring-2 ring-blue-50' : 'border-slate-200 focus:border-slate-300'} rounded-xl px-4 py-3.5 text-slate-900 font-mono tracking-wide focus:outline-none transition-all shadow-sm`}
                    placeholder="Enter Sponsor ID"
                  />
                  <div className="absolute right-4 top-3.5">
                     {sponsorName === 'Invalid Sponsor' && <XCircle className="text-rose-500" size={20} />}
                     {sponsorName && sponsorName !== 'Invalid Sponsor' && <CheckCircle2 className="text-blue-600" size={20} />}
                  </div>
                </div>
                
                <div className="h-4 mt-2 ml-1">
                    {sponsorName && (
                      <p className={`text-[10px] md:text-[11px] font-black tracking-wide uppercase truncate ${sponsorName === 'Invalid Sponsor' ? 'text-rose-500' : 'text-blue-600'}`}>
              ...
              <div className="space-y-3">
                  ...
                  <div className="relative group z-20">
                    ...
                    <input type="tel" placeholder="Mobile Number" value={mobile} onChange={handleMobileChange} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3.5 pl-12 text-slate-900 font-bold placeholder-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all font-mono shadow-sm" />
                  </div>

                  {/* 🔥 NEW FIELD: USDT BEP20 Wallet Address */}
                  <div className="relative group z-10">
                    <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                        <Wallet className="h-5 w-5 text-slate-400 group-focus-within:text-blue-600 transition-colors" />
                    </div>
                    <input 
                      type="text" 
                      placeholder="USDT BEP20 Address (Starts with 0x)" 
                      value={walletAddress} 
                      onChange={e => setWalletAddress(e.target.value)} 
                      className="w-full font-extrabold bg-white border border-slate-200 rounded-xl px-4 py-3.5 pl-12 text-slate-900 font-mono text-sm placeholder-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-50 outline-none transition-all shadow-sm" 
                    />
                  </div>
              </div>

              <button type="submit" disabled={loading} className={`w-full py-4 mt-6 rounded-xl bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-black text-sm tracking-widest uppercase shadow-[0_10px_20px_-10px_rgba(37,99,235,0.5)] hover:shadow-[0_10px_30px_-10px_rgba(37,99,235,0.7)] transition-all flex items-center justify-center gap-2 ${loading ? 'opacity-70 cursor-wait' : 'hover:-translate-y-1 active:scale-95'}`}>
                {loading ? (
                  <>
      ...
      {/* --- LIGHT THEME SUCCESS MODAL (Neo-Banking Style) --- */}
      {showPopup && registeredData && (
        <div style={modalOverlay}>
           
          <div className="animate-in zoom-in duration-300 relative overflow-hidden" style={modalBox}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-blue-50 blur-[50px]"></div>
            
            <div className="w-20 h-20 rounded-2xl bg-white flex items-center justify-center mx-auto mb-6 relative border border-slate-100 shadow-[0_15px_30px_rgba(37,99,235,0.1)]">
               <div className="absolute inset-0 bg-blue-50 rounded-2xl animate-ping opacity-50"></div>
               <CheckCircle2 size={40} className="text-blue-600 relative z-10" strokeWidth={2.5} />
            </div>
            
            <h2 className="text-2xl font-black text-slate-900 mb-1 tracking-tight">ACCOUNT <span className="text-blue-600">CREATED</span></h2>
            <p className="text-slate-500 text-xs uppercase tracking-widest mb-6 font-bold truncate px-2">Welcome, {registeredData.name}</p>
            ...
            <div className="bg-slate-50 border border-slate-200 p-4 sm:p-5 rounded-2xl mb-6 text-left relative z-10 shadow-inner">
              <div className="flex justify-between items-center mb-4 pb-4 border-b border-slate-200">
                 ...
                 <div className="flex items-center gap-2">
                    <span className="text-blue-700 font-black text-lg sm:text-xl tracking-wider font-mono">{registeredData.userId}</span>
                    <button onClick={() => copyToClipboard(registeredData.userId)} className="text-slate-400 hover:text-blue-600 bg-white border border-slate-200 p-1.5 rounded shadow-sm transition-colors"><Copy size={14}/></button>
                 </div>
              </div>
              <div className="flex justify-between items-center">
                 ...
                 <div className="flex items-center gap-2 max-w-[60%]">
                    ...
                    <button onClick={() => copyToClipboard(registeredData.password)} className="text-slate-400 hover:text-blue-600 bg-white border border-slate-200 p-1.5 rounded shadow-sm flex-shrink-0 transition-colors"><Copy size={14}/></button>
                 </div>
              </div>
            </div>

            <div className="bg-amber-50 border border-amber-200 text-amber-600 text-[9px] sm:text-[10px] p-3 rounded-xl font-bold mb-6 flex items-center justify-center gap-2 uppercase tracking-widest shadow-sm">
               ⚠️ Take a screenshot of these details
            </div>
            
            <button onClick={handlePopupClose} className="w-full py-4 rounded-xl bg-slate-900 text-white font-bold tracking-widest uppercase hover:bg-blue-600 transition-all shadow-lg hover:shadow-blue-500/30">
              PROCEED TO LOGIN
            </button>
          </div>
...
const modalBox = { backgroundColor: '#ffffff', border: '1px solid #e2e8f0', padding: '32px 20px', borderRadius: '24px', width: '92%', maxWidth: '400px', textAlign: 'center', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.25)' };
